...
{
    if( diffLen && ((*bwtParams_)[PARAMETER_ALIGN] == 0) && ((*bwtParams_)[PARAMETER_ORDERING] != 2))
        nText = nText_2;
...
int BCRexternalBWT::buildBCR( const string &file1, const string &fileOut, const BwtParameters *bwtParams )
{
    ...
    else
    {
        ...
        if(diffLen){
            if( (*bwtParams_ )[PARAMETER_ALIGN] == 0 )
                transp.align = 0;
            if( (*bwtParams_)[PARAMETER_ALIGN] == 1) {
                if(( *bwtParams_ )[PARAMETER_ORDERING] == 0) //for RLO keep right
                    transp.align = 0;
                else
                    transp.align = 1;
            }
        ...
        if( ( *bwtParams_ )[PARAMETER_ORDERING] == 2 ){
            if(diffLen == false){
              ...
              //  exit(EXIT_SUCCESS);
                ...
                transp.convert(newFast, cycFilesPrefix, false );
            }
        }else{
            transp.convert(file1, cycFilesPrefix, false);
        }
    ...
    alreadyInsert.resize(nText, false);
    ...
    nText_2 = nText; //for variable length
    nextText = nText; //for left align
    ...
    if ( ( *bwtParams_ )[PARAMETER_SAP_ORDERING] == true )
    {
        ...
        //newSymb contains all the simbols in the cycfile
        ...
        for ( AlphabetSymbol j = 0 ; j < sizeAlphaM1; j++ )
        {
            for ( SequenceNumber i = 0; i < nText; ++i )
            {
                ...
                //if I've found the letter in the alphabet
       ...
       if ((*bwtParams_)[PARAMETER_ALIGN] == 0) InsertFirstsymbols(newSymb, newQual); //right
       if ((*bwtParams_)[PARAMETER_ALIGN] == 1) InsertFirstSymbolsLeft(newSymb, newQual); //left
    }
        ...
        #pragma omp parallel sections
        {
            #pragma omp section
            {
                ...
                if ( ( int )currentIteration == nextIterationReset )
                {
                    ...
                    // Standalone block
                    {
                        vector<uchar> newSymb2( nText_2, '$' );
                        assert( newSymb2.size() == nText_2 );
                        assert( newSymb2[nText_2 - 1] == '$' );
...
void BCRexternalBWT::InsertFirstSymbolsLeft(uchar const *newSymb, uchar const *newSymbQual){
    ...
    {
        if( (newSymb[j] != DUMMY_CHAR) && (newSymb[j] != TERMINATE_CHAR)) {

            vectTriple[nText_increasing] = sortElement(0, $posN+1, j);
            nText_increasing++;
            $posN++;
            newSymb_2[i] = newSymb[j];
            alreadyInsert[j] = true;
...
void BCRexternalBWT::InsertFirstsymbols( uchar const *newSymb, uchar const *newSymbQual, const int subSequenceNum )
{
    ...
    if ( verboseEncode == 1 )
    {
        ...
        if ( bwtParams_->getValue( PARAMETER_GENERATE_LCP ) == true )
        {
            ...
            for ( SequenceNumber g = 0 ; g < nText; g++ )
            {
                cerr << ( int )vectTriple[g].getLcpSucN()  << " "; //ignore TODO
            }
            cerr << endl;
        }
    }
...
if ((*bwtParams_)[PARAMETER_ALIGN] == 1) { //align left
    if (nText_increasing < nText) {
        unsigned rank = 0;
        unsigned startFrom = 0;
        for (SequenceNumber r = startFrom; r < nText; r++) {
            if (bwtParams_->getValue(PARAMETER_ORDERING) == 2) { //se l'ordinamento è per lunghezza
                if ((newSymb[r] == DUMMY_CHAR) || (newSymb[r] == TERMINATE_CHAR)) //se è un # o un $ posso interrompermi
                    break;
            }
            if (alreadyInsert[r] == true) rank++;
            if ((alreadyInsert[r] == false) && (newSymb[r] != TERMINATE_CHAR) &&
                (newSymb[r] != DUMMY_CHAR)) { //Se trovo un dollaro aggiungo la seq
                //aggiungi sequenza
                sortElement newVectTripleItem;
                newVectTripleItem.pileN = 0;
                newVectTripleItem.seqN = r;
                newVectTripleItem.posN = rank + 1;
                rank++;
                nText_increasing++;
                alreadyInsert[r] = true;
                parallelVectTriplePerNewPile[0][0].push_back(newVectTripleItem);
            }
        }
    }
}
...
void BCRexternalBWT::InsertNsymbols_parallelPile( uchar const *newSymb, SequenceLength iterationNum, uchar const *newQual, unsigned int parallelPile, SequenceNumber startIndex, SequenceNumber endIndex, vector< FragmentedVector< sortElement > > &newVectTriplePerNewPile )
{
    ...
    if ( j < endIndex )
    {
        ...
        while ( ( k < endIndex ) && ( vectTriple[k].pileN == currentPile ) ) {
            ...
            if (verboseEncode == 1) {
                cerr << "j-1: Q[" << k << "]=" << (int) vectTriple[k].pileN << " P[" << k << "]="
                     << (LetterNumber) vectTriple[k].posN << " N[" << k << "]=" << (SequenceNumber) vectTriple[k].seqN
                     << "\t";
            ...
            if (toRead > 0) {
                ...
                assert((*pReader)((char *) &foundSymbol, 1) == 1); //Legge il simbolo nel file BWT
            ...
            Logger_if(LOG_FOR_DEBUGGING)
            {
                Logger::out() << "toRead=" << toRead << ", foundSymbol=" << foundSymbol << ", counters=" << counters
                              << endl;
                Logger::out() << "Pila" << (int *) currentPile << endl;
            }
    ...
    if (    (*bwtParams_)[PARAMETER_ALIGN] == 1){
            ...
            if (foundSymbol != TERMINATE_CHAR) {
                ...
                newVectTripleItem.posN = counters.count_[whichPile[(int) foundSymbol]];
                if (verboseEncode == 1)
                    cerr << "\nInit New P[" << k << "]= " << vectTriple[k].posN
                         << endl;
                for (AlphabetSymbol g = 0;
                     g < currentPile; g++)  //I have to count in each pile g= 0... (currentPile-1)-pile
                {
                    newVectTripleItem.posN += tableOcc_[g].count_[whichPile[(int) foundSymbol]];
                    if (verboseEncode == 1) {
                        cerr << "g= " << (int) g << " symbol= " << (int) foundSymbol << " whichPile[symbol]= "
                ...
                newVectTripleItem.pileN = whichPile[(int) foundSymbol];
                if (verboseEncode == 1)
                    cerr << "j  : Q[q]=" << (int) vectTriple[k].pileN << " P[q]="
                         << (LetterNumber) vectTriple[k].posN << " N[q]=" << (SequenceNumber) vectTriple[k].seqN
                         << endl;
...
j = startIndex;
while ( j < endIndex ) {
    //printf("currentPile %u e vectTriple[j].pileN %u\n", currentPile, vectTriple[j].pileN);
    ...
    if ((*bwtParams_)[PARAMETER_ALIGN] == 0) assert(currentPile > 0); // I removed the special case for pile 0
...
if ((*bwtParams_)[PARAMETER_ALIGN] == 1){
    ...
    while ((k < nText) && (vectTriple[k].pileN == currentPile)) {
        if (verboseEncode == 1)
            cerr << "k= " << k << " Q[k]= " << (int) vectTriple[k].pileN << " P[k]= " << vectTriple[k].posN
                 << " cont = " << cont << endl;
        //cerr << "k= " << k << " pileN[k]= " << pileN[k] << " posN[k]= " << posN[k] << endl;
        //So I have to read the k-BWT and I have to count the number of the symbols up to the position posN.
        //As PosN starts to the position 1 and I have to insert the new symbol in position posN[k]
        ...
        if (toRead) {
            Logger_if(LOG_FOR_DEBUGGING)
            Logger::out() << "Start: to Read " << toRead << "\n";
